...
class Room extends EventEmitter {
  ...
  createProtooPeer (peerId, transport) {
    ...
    return this._protooRoom.createPeer(peerId, transport)
      .then((protooPeer) => {
        if (this._mediaRoom) { this._handleProtooPeer(protooPeer) } else { this._pendingProtooPeers.push(protooPeer) }
      })
  }
  ...
  _handleProtooPeer (protooPeer) {
    ...
    protooPeer.on('close', () => {
          ...
          peer:
     {
      ...
      if (peerconnection) { peerconnection.close() } else { mediaPeer.close() }
          ...
          switch (request.method) {
            ...
            case 'disableremotevideo':
            {
              ...
              for (let rtpSender of mediaPeer.rtpSenders) {
                if (rtpSender.kind !== 'video') { continue }
